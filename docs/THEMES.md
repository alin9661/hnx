...
`accent` is the brand/surface role, while `highlight` is the accessible text
accent used by headings and metadata. Overriding `accent` also updates
`highlight` for backward compatibility unless the file explicitly provides a
different `highlight` value.
